...
for ($message_size = $min_message_size, $i = 1;
     ...
     $message_size *= 2, $i += 1){
     @message_sizes = (@message_sizes, $message_size);
}
...
for ($i = 0; $i <= $#ARGV; $i++) {
    if ($ARGV[$i] eq "-h" || $ARGV[$i] eq "-?"){
        ...
        $CL = $server->CreateProcess ("sender", "-h");
        $CL->SpawnWaitKill ($server->ProcessStartWaitInterval());

        $SV = $server->CreateProcess ("receiver", "-h");
...
for $protocol (@protocols){
    for $rate (@rates){
        for $message_size (@message_sizes){
            ...
            @configs = (@configs, "-i $iterations -p $protocol -r $rate -s $message_size");
            $i++;
        }
    }
}
...
for $config (@configs){
    $program = "./sender". "-a $test -t $print_statistics $config $extra_args";
    print  "$program \n";
    system ($program);
}
