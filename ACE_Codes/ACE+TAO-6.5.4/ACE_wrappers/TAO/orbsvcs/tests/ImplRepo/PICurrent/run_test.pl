...
foreach $i (@ARGV) {
    if ($i eq '-debug') {
        ...
        $svr_debug = "-ORBDebugLevel 10 -ORBVerboseLogging 1 -ORBLogFile svr.log ";
        $clt_debug = "-ORBDebugLevel 10 -ORBVerboseLogging 1 -ORBLogFile clt.log ";
    }
}

my $extra_timeout = 345;
...
my $c1_srviorfile = $c1->LocalFile ($test_ior);
my $c1_conffile = $c1->LocalFile ($c1_conf);
my $act_srviorfile = $ti->LocalFile ($test_ior);
...
sub delete_files
{
    ...
    $act->DeleteFile ($test_ior);
    $c1->DeleteFile ($test_ior);
}
...
$C1 = $c1->CreateProcess ("client", $client_cmdline);
...
my $srv_cmdline = "$srv_server $svr_debug -orbobjrefstyle url -ORBUseIMR 1 $actinitref";
...
if (ti_cmd ("ior", "-f $act_srviorfile")) {
    ...
    if ($act->WaitForFileTimed ($test_ior,$act->ProcessStartWaitInterval() + $extra_timeout) == -1) {
        kill_imr ("cannot find file <$act_srviorfile>");
        exit 1;
    }

    if ($act->GetFile ($test_ior) == -1) {
        kill_imr ("cannot retrieve file <$act_srviorfile>");
        exit 1;
    }

    if ($c1->PutFile ($test_ior) == -1) {
